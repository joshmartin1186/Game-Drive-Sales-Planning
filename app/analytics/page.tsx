  ...
  const fetchPerformanceData = useCallback(async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        const uniqueProducts = [...new Set(data.map(row => row.product_name).filter(Boolean))]
        const uniqueRegions = [...new Set(data.map(row => row.region).filter(Boolean))] as string[]
        const uniquePlatforms = [...new Set(data.map(row => row.platform).filter(Boolean))]
    ...
    try {
      ...
      for (let i = 0; i < dataRows.length; i += batchSize) {
        ...
        const records = batch.map(line => {
          ...
          const record: Record<string, any> = { client_id: clientId }
