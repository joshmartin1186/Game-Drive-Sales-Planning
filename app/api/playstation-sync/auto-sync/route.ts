...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error('Error fetching PlayStation auto-sync status:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'enable') {
      ...
      // Create an initial sync job immediately
      const today = new Date().toISOString().split('T')[0];
      await supabase
        .from('sync_jobs')
        .insert({
          client_id,
          job_type: 'playstation_sync',
          status: 'pending',
          start_date,
          end_date: today,
      ...
      // Cancel any pending PlayStation auto-sync jobs
      await supabase
        .from('sync_jobs')
        .update({ status: 'cancelled' })
        .eq('client_id', client_id)
        .eq('job_type', 'playstation_sync')
    ...
    console.error('Error managing PlayStation auto-sync:', error);
    return NextResponse.json(
