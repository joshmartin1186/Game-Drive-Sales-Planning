...
// Cache invalidation: 2026-01-08T19:00:00Z - Debug logging for Issue #2
...
export default function GanttChart(props: GanttChartProps) {
  ...
  const { months, days, totalDays } = useMemo(() =&gt; {
    ...
    // DEBUG: Log timeline construction
    console.log('DEBUG Timeline Construction:', {
      timelineStart: timelineStart.toISOString(),
      days0: daysArr[0]?.toISOString(),
      days9: daysArr[9]?.toISOString(),
  ...
  const getPositionForDate = useCallback((date: Date | string): number =&gt; {
    ...
    // DEBUG: Log positioning calculations for all sales
    console.log('DEBUG getPositionForDate:', {
      inputDate: typeof date === 'string' ? date : date.toISOString(),
      normalizedDate: d.toISOString(),
      days0: days[0]?.toISOString(),
      daysDiff,
      position: daysDiff * DAY_WIDTH
    })
